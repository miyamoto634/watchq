//
//  TicTacToeBeforeInterfaceController.swift
//  WatchQ
//
//  Created by H1-157 on 2015/07/15.
...
class TicTacToeBeforeInterfaceController: PetBaseInterfaceController
{
    //メッセージリプレイス
    ...
    func petTalk()
    {
        ...
        {
            let challengeMessageArray = ["まけないよ！", "ぜったいかつからね！", "%＜ペット名＞に勝てるかな？", "お手柔らかに！", "%＜飼い主＞には負けないよ！", "練習してきたんだ！"]
    ...
    override func finishMessage()
    {
        ...
        {
            //チュートリアル画面
            if statusManager.loadValue("TTTTutorialFlag") == 0
            {
                tutorialGroup.setBackgroundImageNamed("TTThelp_0")
            ...
            presentControllerWithName("tictactoe", context: nil)
    ...
    //チュートリアル用のシーン
    @IBAction func pushMoveNextSceneButton()
    {
        if tutorialImageNum >= 1
        {
            //flagを更新
            statusManager.updateValue("TTTTutorialFlag",target_value: 1)
        ...
        animateWithDuration(0.5) { () -> Void in
            self.tutorialGroup.setBackgroundImageNamed("TTThelp_"+String(self.tutorialImageNum))
        }
    }
